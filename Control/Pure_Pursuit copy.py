...
class PurePursuitController(ControllerBase):
    ...
    def ComputeControlCommand(
        ...
        """
        统一控制接口，输入输出均为dict。
        :param state: {'x':..., 'y':..., 'yaw':..., 'v':..., 'direct':...}
        :param reference: {'cx': [...], 'cy': [...], 'target_speed': float, ...}
        :return: {'acceleration':..., 'steer':..., 'target_ind':...}
        """
        ...
        # 速度控制
        acceleration = pid_control(target_speed, node.v, dist, state["direct"])
        # 纯跟踪控制
        delta, self.target_ind = pure_pursuit(node, self.ref_path, self.target_ind)
        return {
...
def example_usage():
    ...
    reference = {"cx": x[0], "cy": y[0], "target_speed": 30.0 / 3.6}
    # 单步调用
    output = controller.ComputeControlCommand(state, reference)
    print("控制输出:", output)
