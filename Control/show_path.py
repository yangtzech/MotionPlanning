...
for seg_id, (cx, cy, cyaw, ccurv, cdirect) in enumerate(zip(x, y, yaw, cur, direct)):
    for i in range(len(cx)):
        print(
            f"Segment {seg_id}, Point {i}: x={cx[i]:.2f}, y={cy[i]:.2f}, yaw={cyaw[i]:.2f}, curvature={ccurv[i]:.4f}, direction={cdirect[i]}"
        )
...
for seg_id, (cx, cy, cyaw, ccurv, cdirect) in enumerate(zip(x, y, yaw, cur, direct)):
    color = (
        "blue" if cdirect[0] == 1 else "red"
    )  # 前进蓝色，后退红色（假设 direction 为 1 或 -1）
    plt.plot(
        cx, cy, color=color, linewidth=2, label=f"Segment {seg_id} (dir={cdirect[0]})"
    )
...
for seg_id, (cx, cy, cyaw, ccurv, cdirect) in enumerate(zip(x, y, yaw, cur, direct)):
    for i in range(0, len(cx), max(1, len(cx) // 5)):  # 每段取少量点显示曲率
        plt.text(
            cx[i] + 0.5, cy[i] + 0.5, f"{ccurv[i]:.4f}", fontsize=10, color="purple"
        )
